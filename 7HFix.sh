...
[ ! -d "$FF7_LOCATION" ] && FF7_LOCATION=$(zenity --width=500 --entry \
  --title="FF7 Location" \
  --text="Couldn't detect a FINAL FANTASY VII installation on this device.\n
...
--title="Instructions" \
--text="Time to install 7th Heaven and configure it for the Steam Deck!\n
...
APP_ID=$(protontricks -s $SEVENTH_HEAVEN_APP_NAME 2> /dev/null | grep -P "Non-Steam shortcut: $SEVENTH_HEAVEN_APP_NAME \([0-9]+\)" | grep -Po "(?<=\()[0-9].+(?=\))")
...
zenity --width=500 --info \
--title="Done!" \
--text="7th Heaven has been successfully installed!\n
<b>******* IMPORTANT *******
